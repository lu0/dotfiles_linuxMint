...
# Search for an emoji and paste it using Rofi and Xmodmap

LIST="🙈 See-No-Evil Monkey
🙉 Hear-No-Evil Monkey
...
🥛 Glass of Milk food
☕ Hot Beverage food
🍵 Teacup Without Handle food
🍶 Sake food
🍾 Bottle with Popping Cork food
🍷 Wine Glass food
🍸 Cocktail Glass food
🍹 Tropical Drink food
🍺 Beer Mug food
🍻 Clinking Beer Mugs food
🥂 Clinking Glasses food
🥃 Tumbler Glass food
🥤 Cup with Straw food
🧃 Beverage Box food
🧉 Mate food
...
selected=$(echo -e "$LIST" | 
           rofi -font "WeblySleek UI Normal, 13" \
                -separator-style none -hide-scrollbar -columns 4 \
                -color-window "#2A2A2A, #8D9194" \
                -color-normal "#2A2A2A, #FFFFFF, #2A2A2A, #3E3F40, #FFFFFF" \
                -p "Search" -dmenu -i);

emoji=$(echo -n "$selected" | cut -d ' ' -f 1);
